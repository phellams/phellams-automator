...
[string]$node_version = try { $node_version = (node --version 2>$null | Out-String).Trim().split(" ")[2] } catch { $node_version = "Unknown" }
[string]$npm_version = try { $npm_version = (npm --version 2>$null | Out-String).Trim().split(" ")[2] } catch { $npm_version = "Unknown" }
[string]$bun_version = try { $bun_version = (bun --version 2>$null | Out-String).Trim().split(" ")[2] } catch { $bun_version = "Unknown" }
...
[string]$av = "Unknown"
try { $av = (Get-CoventionalCommitVersion 2>$null).Version } catch {}
...
$logo = $logo -ireplace "\[node-version\]", "$(csole -s "$($av.Trim())" -c yellow)"
$logo = $logo -ireplace "\[npm-version\]", "$(csole -s "$($av.Trim())" -c yellow)"
$logo = $logo -ireplace "\[bun-version\]", "$(csole -s "$($av.Trim())" -c yellow)"
...
$logo = $logo -ireplace "\[conventionalcommitversion-version\]", "$(csole -s "$(gmv('conventionalcommitversion'))" -c yellow)"
...
# Apply Color Replacements to Shell
