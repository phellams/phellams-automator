...
Function New-NupkgPackage() {
    ...
    process{
        try{
            ...
            Write-QuickLog -Message "[{cs:magenta:nuget}]|-@{pt:{run=pack}} @{pt:{Package=$($nuspecfile.package.metadata.id)}} @{pt:{Version=$($nuspecfile.package.metadata.version)}}" `
                            -Name $global:LOGTASTIC_MOD_NAME `
                            -Type "action"
            Write-QuickLog -Message "creating {cs:magenta:nupkg} package from {cs:magenta:nuspec} file" -Name $global:LOGTASTIC_MOD_NAME -Type "action" -Submessage      
            Write-QuickLog -Message "checking {cs:yellow:nuget} package manager" -Name $global:LOGTASTIC_MOD_NAME -Type "info" -Submessage


            $nuget = Get-Command -Name nuget -ErrorAction SilentlyContinue
            if ($nuget.source.length -eq 0) { 
                throw [System.Exception]::new("nuget package manager not found @{pt:{DownloadFrom=https://www.nuget.org/downloads}}", "nuget.exe not function PATH System variable"); 
            }
