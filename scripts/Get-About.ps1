# Defines the detailed environment report exposed by Get-About and About.
...
if (-not (Test-Path $clapPath -ErrorAction SilentlyContinue)) {
    $clapPath = [System.IO.Path]::Combine(
...
if (-not (Test-Path $tokenizerPath -ErrorAction SilentlyContinue)) {
    $tokenizerPath = [System.IO.Path]::Combine(
        [System.IO.Path]::GetDirectoryName($PSScriptRoot),
        'includes',
        'modules',
        'asciitokenizer.ps1'
...
function Get-About {
    <#
    .SYNOPSIS
    Displays the detailed Phellams Automator environment report.

    .DESCRIPTION
    Renders the full ASCII mascot together with operating-system, binary, and
    PowerShell module versions. The report is opt-in so routine shell startup
    and CI logs remain compact.
...
if (-not (Test-Path $templatePath -ErrorAction SilentlyContinue)) {
    # Fallback to the repository layout for local development and testing.
    $templatePath = [System.IO.Path]::Combine(
        [System.IO.Path]::GetDirectoryName($PSScriptRoot),
        'includes',
        'acsiilogo-template.txt'
    )
}
...
Set-Alias -Name About -Value Get-About -Scope Global
